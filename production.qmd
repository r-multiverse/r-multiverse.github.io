...
The R-multiverse Production tier is a subset of [Community](community.md) package releases
...
Example:^[Caveat: when setting two URLs in the repos argument, the same package may be at both URLs, and the highest version is always preferred.]
...
Production draws from an intermediate tier called [Staging](#staging).
The [Staging](#staging) tier is a [universe](https://staging.r-multiverse.org), and it is active during the month-long period prior to each snapshot.
...
In addition, each package has an RSS feed that updates on each new package release to each tier:^[except in Staging when Staging is currently inactive.]
